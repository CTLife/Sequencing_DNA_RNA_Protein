...
my $HELP = '
        ------------------------------------------------------------------------------------------------------------------------------------------------------
        ------------------------------------------------------------------------------------------------------------------------------------------------------
        Step 4: Mapping reads to the reference genome by using BWAmem or Bowtie2.
        ...
        Usage:
               perl DNA_ChIRP-Seq_4.pl    [-version]    [-help]   [-genome RefGenome]    [-in inputDir]    [-out outDir]  
        For instance:
               nohup time  perl DNA_ChIRP-Seq_4.pl   -genome hg38   -in 3_removedDups   -out 4_rawBAM     > DNA_ChIRP-Seq_4.runLog.txt  2>&1    &  
...
## Version Infromation
my $version = "    The 4th Step, version 1.1,  2023-03-05.";
...
if($boole == 1) {
    ...
    say  "\tPlease see help message by using 'perl  DNA_ChIRP-Seq_4.pl  -help' \n";
    exit 0;
}
...
sub  myQC_BAM_1  {
    ...
    for ( my $i=0; $i<=$#Files; $i++ ) {
        ...
        system( "samstat  --plotend  --norm  --verbose   $dir1/$temp.bam      >> $samstat/$temp.runLog         2>&1");
...
for (my $i=0; $i<=$#pairedEnd_g; $i=$i+2) {
        ...
        system("bwa  mem  -t $numCores_g    -L 3,3   -T 1    $BWA_index_g   $input_g/$end1  $input_g/$end2    > $BWA_g/$temp.sam");
...
for (my $i=0; $i<=$#singleEnd_g; $i++) {
        ...
        $temp =~ s/\.gz//;
        $temp =~ s/\.bz2//;
        system("bwa mem  -t $numCores_g        -L 3,3   -T 1        $BWA_index_g   $input_g/$singleEnd_g[$i]   > $BWA_g/$temp.sam"); 
}

&myQC_BAM_1($BWA_g);
&myMakeDir("$output2_g/1_MultiQC_BWA");
system( "multiqc    --title BWA    --dirs-depth 1    --verbose  --export   --outdir $output2_g/1_MultiQC_BWA      --pdf      $BWA_g     >> $output2_g/1_MultiQC_BWA/MultiQC.BWA.runLog    2>&1" );
...
system( "multiqc    --title Bowtie2    --dirs-depth 1        --verbose  --export   --outdir $output2_g/2_MultiQC_Bowtie2     --ignore QC_Results   --pdf    $Bowtie2_g     >> $output2_g/2_MultiQC_Bowtie2/MultiQC.Bowtie2.runLog    2>&1" );
...
system( "multiqc    --title Bowtie2     --dirs-depth 1       --verbose  --export   --outdir $output2_g/3_MultiQC_Bowtie2_Local     --ignore QC_Results   --pdf    $Bowtie2_g2     >> $output2_g/3_MultiQC_Bowtie2_Local/MultiQC.Bowtie2_Local.runLog    2>&1" );
