...
for (my $i=0; $i<=$#pairedEnd_g; $i=$i+2) {
        ...
        say  tempFH  "$end1,   $end2\n";
        system("STAR  --runMode alignReads    --runThreadN $numCores_g                 --alignSJDBoverhangMin 2   --outSAMmultNmax 1   --readFilesCommand zcat     --outFilterMismatchNoverReadLmax $mismatch_g  --outFilterMismatchNmax 999    --outFilterMultimapNmax 20  --outFileNamePrefix  $STAR_3_g/$temp.   --genomeDir $STAR_index_g  --readFilesIn $input_g/$end1  $input_g/$end2   >> $STAR_3_g/$temp.runLog  2>&1 ");                                                                 
