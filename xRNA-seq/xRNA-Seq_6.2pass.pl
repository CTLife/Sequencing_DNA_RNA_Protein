...
my $output_g   = '';  ## such as "6_rawBAM.2pass"
...
my $HELP = '
        ...
        Usage:
               perl  xRNA-Seq_6.2pass.pl    [-version]    [-help]   [-genome RefGenome]    [-in inputDir]    [-out outDir]  [-mis mismatch] 
        For instance:
               nohup time  perl  xRNA-Seq_6.2pass.pl   -genome hg38   -in 3_removedDups   -out 6_rawBAM.2pass/PE  -mis 0.05    > xRNA-Seq_6.2pass.PE.runLog  2>&1  &
               nohup time  perl  xRNA-Seq_6.2pass.pl   -genome hg38   -in 4_SE-RC         -out 6_rawBAM.2pass/SE  -mis 0.05    > xRNA-Seq_6.2pass.SE.runLog  2>&1  &
...
$output_g = '6_rawBAM.2pass';       ## This is only an initialization value or suggesting value, not default value.
...
if($boole == 1) {
    ...
    say  "\tPlease see help message by using 'perl  xRNA-Seq_6.2pass.pl  -help' \n";
    exit 0;
}
...
###################################################################################################################################################################################################
## Context specific:
my  $commonPath_g      = "/media/yp/1one/MyProgramFiles_02302023/2_Aligners";
...
my $GTF = "/media/yp/1one/MyProgramFiles_02302023/14_Genomes";
...
my $BED12_g = "/media/yp/1one/MyProgramFiles_02302023/14_Genomes";
...
for (my $i=0; $i<=$#pairedEnd_g; $i=$i+2) {
        ...
        say  tempFH  "$end1,   $end2\n";
        system("STAR  --runMode alignReads    --runThreadN $numCores_g           --limitSjdbInsertNsj 9999999   --sjdbFileChrStartEnd     6_rawBAM/PE/3_STAR/*.SJ.out.tab    --alignSJDBoverhangMin 2   --outSAMmultNmax 1   --readFilesCommand zcat     --outFilterMismatchNoverReadLmax $mismatch_g  --outFilterMismatchNmax 999    --outFilterMultimapNmax 20  --outFileNamePrefix  $STAR_3_g/$temp.   --genomeDir $STAR_index_g  --readFilesIn $input_g/$end1  $input_g/$end2   >> $STAR_3_g/$temp.runLog  2>&1 ");                                                                 
...
for (my $i=0; $i<=$#singleEnd_g; $i++) {
        ...
        $temp =~ s/\.gz//;
        $temp =~ s/\.bz2//;
        system("STAR  --runMode alignReads   --runThreadN $numCores_g            --limitSjdbInsertNsj 9999999   --sjdbFileChrStartEnd     6_rawBAM/SE/3_STAR/*.SJ.out.tab    --alignSJDBoverhangMin 2   --outSAMmultNmax 1   --readFilesCommand zcat     --outFilterMismatchNoverReadLmax $mismatch_g    --outFilterMismatchNmax 999    --outFilterMultimapNmax 20     --outFileNamePrefix  $STAR_3_g/$temp.   --genomeDir $STAR_index_g  --readFilesIn $input_g/$singleEnd_g[$i]   >> $STAR_3_g/$temp.runLog  2>&1 ");                                                                                       
        system("rename  s/Aligned.out.sam/sam/   $STAR_3_g/*Aligned.out.sam");
}
