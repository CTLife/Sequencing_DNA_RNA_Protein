...
my $HELP = '
        ...
        ------------------------------------------------------------------------------------------------------------------------------------------------------ 
        Usage:
               perl  xRNA-Seq_8.pl    [-version]    [-help]    [-in inputDir]    [-out outDir]
        For instance:
               nohup time  perl  xRNA-Seq_8.pl    -in 7_finalBAM/PE/3_STAR   -out 8_BW/PE/3_STAR    > xRNA-Seq_8.runLog  2>&1   &
...
if($boole == 1) {
    ...
    say  "\tPlease see help message by using 'perl  xRNA-Seq_8.pl  -help' \n";
    exit 0;
}
...
for ( my $i=0; $i<=$#inputFiles_g; $i++ ) {
        ...
        $temp =~ s/.bam$//  or  die;
        system(" bamCoverage   --bam $input_g/$temp.bam     --outFileName $output_g/$temp.bw     --outFileFormat bigwig       --binSize 10    -p 8     --verbose   --normalizeUsing RPKM   --ignoreForNormalization chrX chrY chrM         > $output_g/$temp.runLog.txt  2>&1  ");                                                  
}
