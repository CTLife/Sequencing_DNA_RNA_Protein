...
my $HELP = '
        ...
        Usage:
               perl DNA_ChIRP-Seq_2.pl    [-version]    [-help]    [-in inputDir]    [-out outDir]
        For instance:
               nohup time  perl DNA_ChIRP-Seq_2.pl   -in 1_rawFASTQ   -out 2_removedAdapters    > DNA_ChIRP-Seq_2.runLog.txt  2>&1    &
...
## Version Infromation
my $version = "    The 2nd Step, version 1.1,  2023-03-05.";
...
if($boole == 1) {
    ...
    say  "\tPlease see help message by using 'perl DNA_ChIRP-Seq_2.pl  -help' \n";
    exit 0;
}
...
for (my $i=0; $i<=$#pairedEnd_g; $i=$i+2) {
        ...
        system("    java  -jar  $Trimmomatic_g  PE   -threads $numCores_g  -summary  $output2_g/Trimmomatic/$temp.statsSummaryFile.txt      $input_g/$end1  $input_g/$end2    $output_g/Trimmomatic/$end1  $output_g/Trimmomatic/unpaired-$end1    $output_g/Trimmomatic/$end2  $output_g/Trimmomatic/unpaired-$end2      ILLUMINACLIP:$Trimmomatic2_g:2:30:10:3:TRUE   SLIDINGWINDOW:4:15  MINLEN:15    >> $output2_g/Trimmomatic/$temp.Trimmomatic.runLog  2>&1");                                                   
...
for (my $i=0; $i<=$#singleEnd_g; $i++) {   
        ...
        system("  java  -jar  $Trimmomatic_g  SE   -threads $numCores_g  -summary  $output2_g/Trimmomatic/$temp.statsSummaryFile.txt   $input_g/$singleEnd_g[$i]  $output_g/Trimmomatic/$singleEnd_g[$i]    ILLUMINACLIP:$Trimmomatic2_g:2:30:10:3   SLIDINGWINDOW:4:15  MINLEN:15      >> $output2_g/Trimmomatic/$temp.Trimmomatic.runLog.txt  2>&1");                    
