...
my $output_g = '';  ## such as "7_BW/3_STAR"
...
my $HELP = '
        ...
        ------------------------------------------------------------------------------------------------------------------------------------------------------ 
        Usage:
               perl  RNA_mRNA-Seq_7.pl    [-version]    [-help]    [-in inputDir]    [-out outDir]
        For instance:
               nohup time  perl  RNA_mRNA-Seq_7.pl    -in 6_finalBAM/3_STAR   -out 7_BW/3_STAR    > RNA_mRNA-Seq_7.runLog  2>&1   &
...
## Version Infromation
my $version = "   The 7th Step, version 1.1,  2023-03-05";
...
$output_g = '7_BW/3_STAR';        ## This is only an initialization value or suggesting value, not default value.
...
if($boole == 1) {
    ...
    say  "\tPlease see help message by using 'perl  RNA_mRNA-Seq_7.pl  -help' \n";
    exit 0;
}
...
for ( my $i=0; $i<=$#inputFiles_g; $i++ ) {
        ...
        $temp =~ s/.bam$//  or  die;
        system(" bamCoverage   --bam $input_g/$temp.bam     --outFileName $output_g/$temp.bw     --outFileFormat bigwig       --binSize 10    -p 16     --verbose   --normalizeUsing RPKM   --ignoreForNormalization chrX chrY chrM         > $output_g/$temp.runLog.txt  2>&1  ");                                                  
}
