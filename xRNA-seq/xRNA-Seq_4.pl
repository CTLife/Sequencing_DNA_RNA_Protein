...
my $HELP = '
        ...
        Step 4: Reverse-complement of R1 or R2 of paired-end reads by using reformat.sh of BBMap. 
                ...
                The versions of tools and packages are used in this script (This is shown for reference only. And do not need to match them.): 
                        ...
                        BBMap,         39.01   (reformat.sh      --version) 
                        MultiQC,       1.14    (multiqc --version)
       
        Usage:
               perl xRNA-Seq_4.pl    [-version]    [-help]    [-in inputDir]    [-out outDir]
        For instance:
               nohup  time  perl xRNA-Seq_4.pl   -in 3_removedDups   -out 4_SE-RC    > xRNA-Seq_4.runLog.txt  2>&1      &
...
## Version Infromation
my $version = "    The 4th Step,  version 1.2,  2023-07-25.";
...
$output_g = '4_SE-RC';         ## This is only an initialization value or suggesting value, not default value.
...
if($boole == 1) {
    ...
    say  "\tPlease see help message by using 'perl xRNA-Seq_4.pl  -help' \n";
    exit 0;
}
...
say   "\n\n\n\n\n\n##################################################################################################";
say   "Checking all the necessary tools in this step ......";

sub printVersion  {
    my $software = $_[0];
    system("echo    '##############################################################################'  >> $output2_g/VersionsOftools.txt   2>&1");
    system("echo    '#########$software : '                                                           >> $output2_g/VersionsOftools.txt   2>&1");
    system("$software                                                                                 >> $output2_g/VersionsOftools.txt   2>&1");
    system("echo    '\n\n\n\n\n\n'                                                                    >> $output2_g/VersionsOftools.txt   2>&1");
}
sub fullPathApp  {
    my $software = $_[0];
    say($software);
...
&printVersion(" reformat.sh      --version");
...
{
say    "\n\n\n\n\n\n##################################################################################################";
say    "Remove PCR and optical duplicates by using reformat.sh of BBMap ......"; 
...
for (my $i=0; $i<=$#pairedEnd_g; $i=$i+2) {
        ...
        system( "gzip   $output_g/$temp.fastq" );
...
for (my $i=0; $i<=$#singleEnd_g; $i++) {   
        ...
        #$temp =~ s/\.fastq\.\S+$//   ;
        #$temp =~ s/\.fastq$//    ;
        system( "reformat.sh  in=$input_g/$temp  out=$output_g/$temp  rcomp=t  > $output_g/$temp.runLog.txt 2>&1 ");
        #system( "cp   $input_g/$temp    $output_g/$temp" ); 
        system( "sleep 2s" );                   
} 
